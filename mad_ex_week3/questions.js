...
function selectHighPerformingStudents(students) {
  return students
.filter((s) => s.GPA >= 5 && s.hobbies.includes("coding"))
.map((s) => {
...
function aggregateStudentData(students) {
  ...
  const studentAvgGPA = students.reduce((accumulator, n) => accumulator + n.GPA,0) / studentNum;
  const codingStudents = students.filter((s) => s.hobbies.includes('coding'))
  const codingStudentNum = codingStudents.length;
  const codingStudentGPA = codingStudents.reduce((accumulator, n) => accumulator + n.GPA,0) / codingStudentNum;
...
function swapForm(input) {
  if (input.includes(" ")) {
    ...
    return input
    .split("")
    .map((s) => (s.toUpperCase() === s ? " " + s.toLowerCase() : s))
    .join("");
}
